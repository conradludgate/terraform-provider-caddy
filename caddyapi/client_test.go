...
func Test_EnforceExists(t *testing.T) {
	...
	mockRequest(httpMock, "POST", "http://localhost/config/apps", `{"http":{}}`, "done", http.StatusOK).Once()
...
func Test_EnforceExistsSlice(t *testing.T) {
	...
	mockRequest(httpMock, "GET", "http://localhost/config/apps/http/servers", "", "null", http.StatusOK).Once()
	mockRequest(httpMock, "POST", "http://localhost/config/apps/http/servers", `{"server":{}}`, "done", http.StatusOK).Once()
	mockRequest(httpMock, "POST", "http://localhost/config/apps/http/servers/server/routes", "[]", "done", http.StatusOK).Once()

	assert.Nil(t, c.EnforceExistsSlice("@config/apps/http/servers/server/routes"))
}

func mockRequest(httpMock *mocks.HTTPClient, method, u string, reqBody string, respBody string, code int) *mock.Call {
	...
	return httpMock.On("Do", reqMatcher).Return(resp, nil)
}
